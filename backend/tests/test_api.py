...
def test_create_user():
    response = client.post(
        "/users/",
        json={"username": "johndoe", "email": "john.doe@example.com", "full_name": "John Doe"},
    )
    assert response.status_code == 200
...
def test_update_user():
    response = client.put(
        "/users/johndoe",
        json={"username": "johndoe", "email": "john.doe.updated@example.com", "full_name": "John Doe Updated"},
    )
    assert response.status_code == 200
    assert response.json()["username"] == "johndoe"
...
def test_create_user_with_street1():
    response = client.post(
        "/users/",
        json={"username": "janedoe", "email": "jane.doe@example.com", "full_name": "Jane Doe", "street1": "123 Main St"},
    )
    assert response.status_code == 200
...
def test_update_user_with_street1():
    response = client.put(
        "/users/janedoe",
        json={"username": "janedoe", "email": "jane.doe.updated@example.com", "full_name": "Jane Doe Updated", "street1": "456 Oak Ave"},
    )
    assert response.status_code == 200
    assert response.json()["username"] == "janedoe"
